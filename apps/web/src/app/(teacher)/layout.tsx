...
const navItems = [
  ...
  { href: '/teacher/courses', label: 'Mis cursos', icon: '📚' },
  { href: '/teacher/students', label: 'Estudiantes', icon: '👥' },
  { href: '/teacher/sessions', label: 'Sesiones', icon: '🎥' },
  { href: '/teacher/attendance', label: 'Asistencia', icon: '✅' },
  { href: '/teacher/homework', label: 'Tareas', icon: '📋' },
  { href: '/teacher/grades', label: 'Calificaciones', icon: '📊' },
  { href: '/teacher/materials', label: 'Materiales', icon: '📁' },
...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  const initials = (user?.fullName ?? 'P').split(' ').map((n: string) => n[0]).slice(0, 2).join('').toUpperCase()
    ...
    <div className="flex h-screen overflow-hidden bg-background">
      <aside className="w-52 flex-shrink-0 border-r border-border bg-card flex flex-col">
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
          <p className="text-[10px] text-muted-foreground px-2 py-1 uppercase tracking-wide">Profesor</p>
          {navItems.map(item => (
        ...
        <div className="border-t border-border p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="overflow-hidden">
              <p className="text-xs font-medium text-foreground truncate">{user?.fullName ?? 'Profesor'}</p>
              <p className="text-[10px] text-muted-foreground">Profesor</p>
            </div>
          </div>
